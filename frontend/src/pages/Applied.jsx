...
const Applied = () => {
  ...
  const loadVacatures = async () => {
    try {
      setLoading(true);
      const data = await vacaturesAPI.getAll('toegepast');
      setVacatures(data);
    } catch (error) {
      console.error('Error loading applied vacatures:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerateMotivation = async (vacatureId) => {
    if (!profile) {
      toast.warning(t('pages.applied.completeProfile'));
      return;
    }
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">{t('pages.applied.title')}</h2>
        <p className="text-gray-600">{t('pages.applied.subtitle')}</p>
      </div>

      {vacatures.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500 text-lg">{t('pages.applied.noResults')}</p>
          <p className="text-gray-400 text-sm mt-2">{t('pages.applied.resultsHint')}</p>
        </div>
      )}
